...
> Plantilla de asistente ejecutivo personal con Claude Code.
...
```
├── CLAUDE.md          ← este archivo: CUÁNDO pasa cada cosa
├── User/user.md       ← perfil del usuario
├── Agent/
│   ├── agent.md       ← quién soy y cómo me comporto
│   └── memory/        ← lo que el asistente APRENDE (auto-memoria de Claude Code, versionada)
├── Tools/
...
2. Leer [User/user.md](User/user.md), [Agent/agent.md](Agent/agent.md),
   [Tools/tools.md](Tools/tools.md) y [Memory/index.md](Memory/index.md).
...
- **Una que ya existe:** cuando una tarea podría hacerse mejor con una skill que no está instalada,
  **preguntá antes**: *«Para esto puede haber una skill. ¿Busco una?»*. Con el OK, `find-skills`,
  ...
  `npx skills add <owner/repo@skill> -a claude-code -y`. Registrala en `Tools/tools.md`.
- **Una propia:** cuando un proceso **siempre se hace igual** y ya se hizo dos veces, proponé
  encapsularlo con `skill-creator`. Confirmá antes. No encapsules tareas únicas o creativas.
...
- **`./with-env.sh <comando>`** corre cualquier cosa con el `.env` cargado: servidores MCP (en
  `.mcp.json`, como `"command": "./with-env.sh"`) y scripts propios.
